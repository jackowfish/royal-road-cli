...
var rootCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		// Check if this is first launch (needs theme selection)
		cfg, err := config.Load()
		if err != nil {
		...
		var model tea.Model
		if cfg.NeedsThemeSelection() {
			// Show theme selector on first launch
			model = ui.NewThemeSelectorModel()
		} else {
			// Apply saved theme and show menu
			ui.SetTheme(ui.GetThemeByName(cfg.ThemeName))
